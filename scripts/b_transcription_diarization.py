...
def main():
    ...
    diarization_pipeline._segmentation.model = segmentation_model.to(device)
    ...
    for each in tqdm(data.itertuples(), total=data.shape[0]):
        ...
        if os.path.exists(f'{output_dir}/{each.candidate_name}/{each.id}.json'):
            logger.info(f'File already exists, skipping: "{each.title}"')
            continue
        else:
            logger.info(f'Processing now: "{each.title}"')
        ...
        waveform, sample_rate = torchaudio.load(audio_path) # process from memory

        from pyannote.audio.pipelines.utils.hook import ProgressHook
        with ProgressHook() as hook:
            diarization_results = diarization_pipeline(
                {"waveform": waveform, "sample_rate": sample_rate},
                min_speakers=2, max_speakers=5, hook=hook
            )
        ...
        with open(f"{os.path.splitext(audio_path)[0]}_baseline.json", "w") as f:
            json.dump(ssm, f)

        logger.info(f'Finished transcription for: "{each.url}"')
        ...
        for chunk in ssm:
            assert (chunk['start_time'] / 1000.0) >= 0, "non-negative timestamp expected"
            # extract embedding for a speaker speaking between t=Xs and t=Ys
            logger.info(f"chunk: {chunk}")
            if (chunk["end_time"] / 1000.0) > wav_duration:
                segment_chunk = Segment((chunk['start_time'] / 1000.0), wav_duration)
                logger.info(f"segment_chunk: {segment_chunk}")
            else:
                segment_chunk = Segment((chunk['start_time'] / 1000.0), (chunk['end_time'] / 1000.0))
                logger.info(f"segment_chunk: {segment_chunk}")
            waveform_chunk, sample_rate = io.crop(
                audio_path, segment_chunk)
            logger.info(f"waveform_chunk: {waveform_chunk}")
            # extract embeddings for that chunk
            chunk_embeddings = infer_embeddings.crop(
                {"waveform": waveform_chunk, "sample_rate": sample_rate})
            logger.info(f"chunk_embeddings: {chunk_embeddings}")
            ...
            chunk['Speaker'] = each.candidate_name if distance[0][0] <= cdist_threshold else "entrevistador/a"

        logger.info(f'Finished speaker verification for "{each.candidate_name}"')
